...
router.get('/authorize', async (req, res) => {
	const {client_id, response_type, scope, state, redirect_uri, nonce} = req.query;
	...
	if (client.redirectUri != redirect_uri) {
		...
		res.status(200).send('client identified successfully, Resourceowner can authenticate now!');
	}
});
